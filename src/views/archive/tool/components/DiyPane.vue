<template>
  <div class="tabPane-container">
    ...
    >
      ...
      <el-table-column align="center" label="照片" width="60" prop="toolInfo.photoSrc">
        <template slot-scope="scope">
          <img v-if="scope.row.toolInfo.photoSrc" :src="apiUrl + scope.row.toolInfo.photoSrc" width="25" height="25" />
          <span v-else>未上传</span>
        </template>
      </el-table-column>
      <el-table-column label="名称" align="center" prop="name" sortable="custom">
        <template slot-scope="scope">
          {{ scope.row.toolInfo.name }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="英文名" align="center">
        <template slot-scope="scope">
          {{ scope.row.toolInfo.engName }}
        </template>
      </el-table-column>
      <el-table-column label="日文名" align="center">
        <template slot-scope="scope">
          {{ scope.row.toolInfo.jpnName | jpnFilter }}
        </template>
      </el-table-column>
      ...
      <el-table-column class-name="status-col" label="操作" width="150" align="center">
        <template slot-scope="scope">
          <el-button type="primary" icon="el-icon-edit" size="small" @click="$emit('paneEdit', scope.row._id, 'isDIY')"></el-button>
...
export default {
  ...
  computed: {
    ...
    // toolInfoName() {
    //   let toolInfoN = ''
    //   //console.log(this.diyKey)
    //   if ((this.diyKey = 'isDIY')) {
    //     toolInfoN = 'toolInfo'
    //   } else {
    //     toolInfoN = ''
    //   }
    ...
    //   //console.log(this.diyKey)
    //   if ((this.diyKey = 'isDIY')) {
    //     toolInfoM = 'scope.row.toolInfo'
    //   } else {
    //     toolInfoM = 'scope.row'
    //   }
    //   //console.log(toolInfoM)
    //   return toolInfoM
    // }
  },
  created() {
  ...
  methods: {
    fetchData(param) {
      ...
      getDiyTools(this.queryInfo).then(response => {
